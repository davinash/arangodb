...
AstNode *Ast::createNodeDataSource(arangodb::CollectionNameResolver const &resolver,
    ...
    if (dataSource->category() == LogicalView::category()) {
        // it's a view!
        ...
        if (node->type == NODE_TYPE_PARAMETER ||
            ...
            if (node->type == NODE_TYPE_PARAMETER) {
                ...
                node = createNodeDataSource(resolver, name, l,
                                            isWriteCollection ? AccessMode::Type::WRITE : AccessMode::Type::READ, false,
                                            true
                );
...
std::unordered_set<std::string> Ast::getReferencedAttributesForKeep(AstNode const *node,
    ...
    std::function<bool(
            AstNode const *)> visitor = [&isSafeForOptimization, &result, &isTargetVariable, &searchVariable](
            AstNode const *node) {
...
AstNode *Ast::optimizeAttributeAccess(AstNode *node,
                                      std::unordered_map<Variable const *, AstNode const *> const &variableDefinitions) {
    TRI_ASSERT(node != nullptr);
...
AstNode *Ast::optimizeFunctionCall(AstNode *node) {
    ...
    if (func->name == "LENGTH" || func->name == "COUNT") {
        ...
        if (args->numMembers() == 1) {
            ...
            return createNodeBinaryOperator(NODE_TYPE_OPERATOR_BINARY_EQ, args->getMemberUnchecked(0),
                                            createNodeValueNull());
        }
#if 0
...
AstNode *Ast::createNodeCollectionNoValidation(char const *name,
                                               size_t nameLength,
    ...
    if (ServerState::instance()->isCoordinator()) {
        ...
        // which does not exist, if only for some unit tests:
        try {
            auto coll = ci->getCollection(_query->vocbase().name(), nameString);
