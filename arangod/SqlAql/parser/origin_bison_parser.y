...
opt_from_clause:
        from_clause  {
            $$ = $1;
            std::cout << "SQL::opt_from_clause::$$ = " << $$->name << std::endl;
            auto ast = parser->ast();
            arangodb::aql::AstNode* node = nullptr;
            auto variable = ast->scopes()->getVariable(TRI_CHAR_LENGTH_PAIR($$->name), true);

            if (variable == nullptr) {
                if (ast->scopes()->canUseCurrentVariable() && strcmp($$->name, "CURRENT") == 0) {
                    variable = ast->scopes()->getCurrentVariable();
                }
                else if (strcmp($$->name, arangodb::aql::Variable::NAME_CURRENT) == 0) {
                   variable = ast->scopes()->getCurrentVariable();
                }
            }
            if (variable != nullptr) {
            ...
            if (node == nullptr) {
              ...
              node = ast->createNodeDataSource(resolver, TRI_CHAR_LENGTH_PAIR($$->name), arangodb::AccessMode::Type::READ, true, false);
            }
...
from_clause:
		FROM table_ref {
		    std::cout << "SQL::from_clause::$2 = " << $2 << std::endl;
		    $$ = $2;
		}
	;
...
column_name:
		IDENTIFIER {
		  ...
		  parser->pushStack(parser->ast()->createNodeVariable(TRI_CHAR_LENGTH_PAIR($1), true));
		  std::cout << "SQLParser::bison::column_name clause hit4 DONE " <<  std::endl;
		}
	...
	|	'*' {
	            std::cout << "SQLParser::bison::column_name clause hit2" << std::endl;
	            $$ = Expr::makeStar();
...
table_name:
		IDENTIFIER                {
		//std::cout << "SQL::table_name::$$ " << $$  << std::endl;
		//$$.schema = nullptr; $$.name = $1;
		$$ = parser->ast()->createNodeValueString(TRI_CHAR_LENGTH_PAIR($1));
		}
	//|	IDENTIFIER '.' IDENTIFIER {
	      //$$.schema = $1; $$.name = $3;
	  //    $$ = parser->ast()->createNodeValueString(TRI_CHAR_LENGTH_PAIR($3));
	    //}
	;
